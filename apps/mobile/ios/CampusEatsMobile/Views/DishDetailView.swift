                    ...
                    HStack {
                        ...
                        Button {
                            ...
                            Image(systemName: favoritesManager.isFavorite(dish.dishId) ? "heart.fill" : "heart")
                                .font(.title2)
                                .foregroundColor(favoritesManager.isFavorite(dish.dishId) ? .red : .white)
                                .padding(12)
                        ...
                        Button {
                            ...
                            Image(systemName: showLoggedToast ? "checkmark" : "plus")
                                .font(.title2)
                                .foregroundColor(showLoggedToast ? .green : .white)
                                .padding(12)
                                .background(.ultraThinMaterial)
                                .clipShape(Circle())
                        }
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            FlowLayout(spacing: 8) {
                                ForEach(tags, id: \.self) { tag in
                                    DietaryTagView(tag: tag, size: .medium)
                                }
                            }
                        }
                    }
...
struct ReviewRow: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showReportSheet) {
            ReportReviewSheet(review: review) {
                showReportedAlert = true
            }
        }
        .alert("Report Submitted", isPresented: $showReportedAlert) {
            Button("OK", role: .cancel) { }
        } message: {
            Text("Thank you for helping keep Campus Eats safe. We'll review this report shortly.")
        }
    }
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Review by \(review.userName ?? "Anonymous")")
                            .font(.subheadline)
                            .fontWeight(.medium)
                        
                        if let text = review.textReview {
                            Text(text)
                                .font(.caption)
                                .foregroundStyle(.secondary)
                                .lineLimit(3)
                ...
                Section {
                    ...
                    Text("Please provide any additional context that would help us review this report.")
                }
            }
